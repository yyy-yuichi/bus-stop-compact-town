...
`scripts/build-basemap-style.py` が原本ハッシュを確認して `public/maps/soft.json` を再現する。配色は生成りの陸地・青緑の水面とし、日本語優先で地名を一度だけ表示。小さい地域名の文字を大きくし、鉄道と駅名を補った。元スタイルの道路番号未登録時の比較式も修正している。
