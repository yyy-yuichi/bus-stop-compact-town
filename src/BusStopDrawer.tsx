  ...
  return <aside ref={drawerRef} hidden={hidden} role="dialog" aria-labelledby="bus-drawer-title"
    ...
    <header className="detail-header">
      <div className="min-w-0">
        <p className="detail-eyebrow"><span className={`stop-symbol${municipal ? ' municipal-symbol' : ''}`}><MapIcon name="bus" /></span>{municipal ? `${stop.properties.city}の停留所` : national ? 'バス停' : 'バス停 · 徒歩圏試作'}</p>
    ...
    {!national && !municipal && <button className="drawer-return" onClick={onNational}>← 県全体のバス停に戻る</button>}
    <div className="detail-body">
      {children}
      <SharePlace place={municipal ? { kind: 'municipal', id } : national ? { kind: 'national', id, minutes: bakedMinutes } : { kind: 'pilot', id, walking: walkingConditions }} />
      ...
      {municipal && <p className="mb-5 text-xs leading-relaxed">市が公開した停留所の登録点です。国の代表点との重複は統合していません。乗り場の方向・現在の運行状況・現地の入口は未確認です。</p>}
      <dl className="space-y-6 text-sm">
        {!municipal && <div><dt className="text-xs font-semibold text-stone-500">運行事業者（{national ? '国土数値情報' : 'OSM登録情報'}）</dt>
        ...
        <div><dt className="text-xs font-semibold text-stone-500">{national || municipal ? '出典レコードID' : 'OSM ID'}</dt>
      ...
      <div className="mt-7 rounded-xl bg-stone-50 p-4 text-xs leading-relaxed text-stone-600">
        {!municipal && <p>{national ? '国土数値情報を加工しています。位置・運行情報の現在の正確性は未確認です。' : 'OSM由来の試用データです。国の停留所との対応・上下線は未確認です。'}</p>}
        <p className="mt-2 break-all">データ時点：{municipal ? stop.properties.source_date : timestamp || '不明'}</p>
