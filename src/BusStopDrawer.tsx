    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-6 pt-6 pb-[max(24px,env(safe-area-inset-bottom))]">
      ...
      <dl className="space-y-6 text-sm">
        <div><dt className="text-xs font-semibold text-stone-500">運行事業者</dt>
          <dd className="mt-2 break-words font-medium text-stone-900">{stop.properties?.operators?.length ? stop.properties.operators.join('、') : '登録情報なし'}</dd></div>
        <div><dt className="text-xs font-semibold text-stone-500">系統</dt>
          <dd className="mt-2 break-words font-medium text-stone-900">{stop.properties?.routes?.length ? stop.properties.routes.join('、') : '登録情報なし'}</dd></div>
        <div><dt className="text-xs font-semibold text-stone-500">停留所ID（座標）</dt>
      ...
      <div className="mt-7 rounded-xl bg-stone-50 p-4 text-xs leading-relaxed text-stone-600">
        <p>国土数値情報バス停留所データ（国土交通省）に基づく試用データです。同じ位置の記録は1件にまとめ、名称・事業者・系統を統合しています。位置・運行情報の正確性、最新性は未確認です。</p>
        <p className="mt-2 break-all">データ時点：{timestamp || '不明'}</p>
        <a className="mt-3 inline-block text-sky-800 underline underline-offset-2" href="https://nlftp.mlit.go.jp/ksj/" target="_blank" rel="noreferrer">国土数値情報ダウンロードサイト（国土交通省）</a>
      </div>
      </details>
    </div>
  </aside>;
