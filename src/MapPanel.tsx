...
export default function MapPanel({ stops, facilities, categories, onCategories, onStop, onFacility, mode, onMode, busy, shoppingError, shoppingLoading, retryShopping, selection }: Props) {
  ...
  return <aside className={`map-panel${expanded ? ' is-expanded' : ''}`} aria-label="地図の検索と表示設定">
    ...
    <div className="search-field"><MapIcon name="search" /><label className="sr-only" htmlFor="place-search">バス停・買い物先を検索</label><input id="place-search" type="search" placeholder="バス停・買い物先を検索" value={query} onChange={e => changeQuery(e.target.value)} onKeyDown={e => { if (e.key === 'Escape' && query) { e.stopPropagation(); clearQuery(); } }} autoComplete="off" />{query && <button className="search-clear" aria-label="検索をクリア" onClick={clearQuery}><MapIcon name="close" /></button>}</div>
    {term ? <div className="search-results" ref={resultPanel} aria-label="検索結果"><p className="list-label" role="status">検索結果 {matches.stops.length + matches.facilities.length}件{busy || shoppingLoading ? '（読み込み中）' : ''}</p>
      {matches.facilities.length > 0 && <section aria-labelledby="facility-results-title"><h3 className="result-group-title" id="facility-results-title">買い物先 <span>{matches.facilities.length}件</span></h3>
        {matches.facilities.slice(0, limits.facilities).map(facilityRow)}
        {matches.facilities.length > limits.facilities && <button className="more-results" onClick={() => { pendingFocus.current = { section: 'facility-results-title', index: limits.facilities }; setLimits(p => ({ ...p, facilities: p.facilities + 12 })); }}>買い物先をさらに表示（残り{matches.facilities.length - limits.facilities}件）</button>}
      </section>}
      ...
      {shoppingError && <button className="text-button error-text" onClick={retryShopping}>買い物先を再読み込み</button>}
      ...
      <div className="panel-options" id="map-options">
        <section className="category-section"><div className="section-label"><h3>買い物先</h3><span>表示する種類</span></div>
          ...
          {shoppingError ? <button className="text-button error-text" onClick={retryShopping}>買い物先を再読み込み</button> : shoppingLoading ? <p className="helper-text" role="status">買い物先を読み込み中…</p> : <button className="list-toggle" aria-expanded={listOpen} onClick={() => setListOpen(p => !p)}>登録済みの買い物先 <span>{visible.length}件 <span aria-hidden="true">{listOpen ? '−' : '+'}</span></span></button>}
