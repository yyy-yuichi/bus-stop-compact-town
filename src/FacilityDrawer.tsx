  ...
  return <aside className="detail-drawer facility-drawer" ref={panel} role="dialog" aria-labelledby="facility-title">
    ...
    <div className="detail-body">
      ...
      <div className="soft-note"><strong>地図上の位置について</strong><p>{p.geometry_kind === 'representative_point' ? '施設の代表位置を表示しています。' : p.geometry_kind === 'facility_area' ? '施設の範囲を表示しています。' : '建物の形を表示しています。'}入口や、バス停から歩いて到達できることを確認した表示ではありません。</p></div>
        <dl><dt>名称・所在地の確認</dt><dd>{p.purpose_review ? `${p.purpose_review.checked_at}に上記の範囲を公式資料で確認。位置・入口は未確認` : civic ? '自治体の公開一覧から取り込み・現況未確認' : imported ? '未確認（OSM登録情報を取り込み）' : p.verified_at}</dd><dt>{imported || civic ? '取得元データの基準日時' : '位置データの更新日時'}</dt><dd>{civic ? '自治体オープンデータ' : 'OpenStreetMap'}（{p.source_timestamp}）</dd>{(imported || civic) && <><dt>取り込み日</dt><dd>{p.retrieved_at?.slice(0, 10)}</dd></>}</dl>
