...
export type BasemapMode = 'soft' | 'standard' | 'gsi-pale' | 'gsi-photo';
...
export default function BasemapSettings({ comparison = false }: { comparison?: boolean }) {
  ...
  return <section className="basemap-settings"><h3>地図の見た目</h3><div role="group" aria-label="背景地図">
    ...
    {comparison && <><button aria-pressed={mode === 'gsi-pale'} onClick={()=>onMode('gsi-pale')}>地理院・淡色</button><button aria-pressed={mode === 'gsi-photo'} onClick={()=>onMode('gsi-photo')}>地理院・写真</button></>}
