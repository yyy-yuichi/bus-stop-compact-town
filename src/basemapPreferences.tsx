...
export default function BasemapSettings() {
  ...
  return <section className="basemap-settings"><h3>地図の見た目</h3><div role="group" aria-label="背景地図">
    <button aria-pressed={mode === 'soft'} onClick={()=>onMode('soft')}><span className="basemap-swatch soft-swatch" aria-hidden="true"/>やわらか</button>
  ...
  </div>{fallback && <p className="helper-text" role="status">現在は標準地図で表示しています。「やわらか」を押すと再試行します。</p>}</section>;
}
