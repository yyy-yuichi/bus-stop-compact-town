...
const map=L.map('review-map',{zoomControl:false,preferCanvas:true,minZoom:6,maxZoom:19}).setView([33.99,131.95],12);
...
async function start(){
  const [osm,official,routes]:[BusCollection,Official,Route[]]=await Promise.all([get('bus_stop.geojson'),get('review-stops.geojson'),get('review-routes.json')]);
  if(osm.features.length!==1085||official.features.length!==907||!Array.isArray(routes))throw Error('Unexpected data');
  ...
  const fit=(area:string)=>{map.closePopup();const points=official.features.filter(f=>area==='all'||f.properties.source_namespace===area).map(f=>L.latLng(f.geometry.coordinates[1],f.geometry.coordinates[0]));map.fitBounds(L.latLngBounds(points),{paddingTopLeft:[25,165],paddingBottomRight:[55,65],maxZoom:14,animate:false});document.querySelectorAll<HTMLButtonElement>('[data-area]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.area===area)));};
  document.querySelectorAll<HTMLButtonElement>('[data-area]').forEach(b=>b.onclick=()=>fit(b.dataset.area!));
  for(const [id,layer] of [['show-osm',existing],['show-official',added]] as const)el<HTMLInputElement>(id).onchange=()=>{map.closePopup();if(el<HTMLInputElement>(id).checked)layer.addTo(map);else layer.remove();if(map.hasLayer(added))added.bringToFront();};
  status.hidden=true;fit('hikari');
}
