...
最新の機能：[徒歩5分・10分のまち（試作）](docs/walking-pilot-20260908.md)。おのだサンパーク周辺の7停留所で、道路に沿った徒歩圏と買い物候補を表示。時速3/4kmを切替可能。入口までの経路は未確認。以下の「徒歩到達圏未対応」は以前の実装履歴です。

現在のUI：バス停・商業施設の操作パネルと「ドラッグで移動…」の案内は削除済み。バス停はクリックで右ドロワー、商業施設はクリックで詳細を表示する。以下の一覧選択・表示切替に関する記録は削除前の実装履歴。データ出典リンクと読込失敗時の再試行は残す。
