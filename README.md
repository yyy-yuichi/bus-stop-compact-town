...
最新（2026-09-13）：岩国市735件・光市172件の市データを通常マップの表示・検索・共有に追加。国の代表点を基本に、市の登録点は原本ID・座標を保った別の点として表示する。市の点の徒歩圏を国の結果で代用せず、近くの国の起点へ明示的に移動できる。地域移動ボタンを常設し、設定・注意文・出典・凡例は初期状態で閉じる。地図上の名前・時間・施設候補を中心にし、説明を常設した大きなポップアップを増やさないというユーザーの継続要望を優先する。

国のバス停の徒歩圏は施設候補と5分・10分・15分切替に対応。買い物・医療別に候補を表示し、施設詳細から元の徒歩条件へ戻れる。共有リンクと再読み込みで時間を復元する。[徒歩圏と施設の照合資料](data-sources/walking-facilities-20260913/README.md)を参照。時間は近くの道路までの目安で、施設の入口への通行可否は未確認。
