...
最新（2026-09-13）：国のバス停の徒歩圏に施設候補と5分・10分・15分の切替を追加。買い物・医療別に候補を表示し、施設詳細から元の徒歩条件へ戻れる。共有リンクと再読み込みで時間を復元する。[徒歩圏と施設の照合資料](data-sources/walking-facilities-20260913/README.md)を参照。時間は近くの道路までの目安で、施設の入口への通行可否は未確認。

[検索・共有リンク](docs/integration-ready-20260911.md)と施設1,135レコードのID・分類・確認状態を維持。新しい徒歩圏はPR #1で受領し、国の4,418レコードを同じ原資料の3,946計算起点へ対応付け済み。3,657起点の計算済み道路はR2で配信し、接続不可289起点は理由を表示する。[従来の試作](docs/walking-pilot-20260908.md)は別のOSM7停留所・おのだサンパーク1施設を対象として保持する。通常7分類の1,130施設レコードから候補を抽出し、参考5件は除く。現在の公開・保存状況は [HANDOFF](docs/HANDOFF.md) を参照。
