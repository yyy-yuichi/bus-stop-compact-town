...
最新（2026-09-11）：[OSM施設の取り込みとアイコン](docs/osm-facilities-20260911.md)。バス停4,418件と施設1,135件を表示・検索・選択・共有できる。スーパーは買い物かごのアイコンに変更。ドラッグストア、コンビニ、商業施設、病院、診療所、薬局を含む7種類で切り替える。追加施設はOSM登録情報として扱い、個別に照合した既存22件と確認状態を区別する。

[検索・共有リンク](docs/integration-ready-20260911.md)の機能を維持。今回の施設IDを含む一覧は `outputs/osm-facilities-20260911/id-index/` に生成する。[徒歩5分・10分の試作](docs/walking-pilot-20260908.md)は引き続きOSM7停留所・おのだサンパーク1施設が対象。追加施設への徒歩経路や国の停留所との対応は未実装。新しい徒歩圏データも未受領。
