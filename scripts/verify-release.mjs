...
for(const file of ['bus_stop.geojson','shopping.geojson','civic-facilities.geojson','baked-bus-stops.geojson','walk-unreachable.json','facility-current.json']) if(!fs.readFileSync(`public/data/${file}`).equals(fs.readFileSync(path.join(root,'data',file))))throw Error(`Stale data: ${file}`);
