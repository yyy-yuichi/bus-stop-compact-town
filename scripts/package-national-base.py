...
out=root/'outputs/national-base-20260911';out.mkdir(parents=True,exist_ok=True)
...
source.update((root/'work/national-base-20260911').glob('*.png'))
...
report=[]
for name,files,base in [('national-base-source.zip',source,root),('national-base-site.zip',set((root/'dist').rglob('*')),root/'dist')]:
    ...
    with zipfile.ZipFile(archive) as z:
        ...
        for p,d in manifest.items():assert hashlib.sha256(z.read(p)).hexdigest()==d
