...
assert.equal(current.length, 7901);
assert.equal(patch.updates.length, 24);
assert.equal(patch.additions.length, 137);
...
assert.equal(patch.additions.filter(f => f.properties.freshness_review.event === 'listed').length, 125);
...
assert.equal(current.filter(f => f.properties.category !== 'reference' && facilityAvailable(f)).length, 7840);
...
console.log(JSON.stringify({ original_records: raw.length, current_records_including_history: current.length, latest_local_batch_additions: 72, cumulative_updates: patch.updates.length, cumulative_additions: patch.additions.length, cumulative_closures: 6, invalid_overlays_rejected: rejected, extra_partial_date_rejections: 9, raw_ID_geometry_service_history_preserved: true, search_nearby_walk_map_checks: 'passed' }));
