...
assert.equal(current.length, 7943);
assert.equal(patch.updates.length, 30);
assert.equal(patch.additions.length, 179);
...
assert.equal(patch.additions.filter(f => f.properties.freshness_review.event === 'listed').length, 164);
...
assert.equal(current.filter(f => f.properties.category !== 'reference' && facilityAvailable(f)).length, 7879);
