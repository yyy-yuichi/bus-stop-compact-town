...
assert.equal(current.length, 8044);
assert.equal(patch.updates.length, 41);
assert.equal(patch.additions.length, 280);
...
assert.equal(patch.additions.filter(f => f.properties.freshness_review.event === 'listed').length, 265);
...
assert.equal(current.filter(f => f.properties.category !== 'reference' && facilityAvailable(f)).length, 7980);
