...
assert.equal(current.length, 8006);
assert.equal(patch.updates.length, 31);
assert.equal(patch.additions.length, 242);
...
assert.equal(patch.additions.filter(f => f.properties.freshness_review.event === 'listed').length, 227);
...
assert.equal(current.filter(f => f.properties.category !== 'reference' && facilityAvailable(f)).length, 7942);
