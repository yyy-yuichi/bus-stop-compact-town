...
assert.equal(current.length, 7909);
assert.equal(patch.updates.length, 24);
assert.equal(patch.additions.length, 145);
...
assert.equal(patch.additions.filter(f => f.properties.freshness_review.event === 'listed').length, 133);
...
assert.equal(current.filter(f => f.properties.category !== 'reference' && facilityAvailable(f)).length, 7848);
