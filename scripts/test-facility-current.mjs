...
assert.equal(current.length, 8001);
assert.equal(patch.updates.length, 30);
assert.equal(patch.additions.length, 237);
...
assert.equal(patch.additions.filter(f => f.properties.freshness_review.event === 'listed').length, 222);
...
assert.equal(current.filter(f => f.properties.category !== 'reference' && facilityAvailable(f)).length, 7937);
