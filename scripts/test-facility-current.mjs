...
assert.equal(current.length, 8020);
assert.equal(patch.updates.length, 31);
assert.equal(patch.additions.length, 256);
...
assert.equal(patch.additions.filter(f => f.properties.freshness_review.event === 'listed').length, 241);
...
assert.equal(current.filter(f => f.properties.category !== 'reference' && facilityAvailable(f)).length, 7956);
