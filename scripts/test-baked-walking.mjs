...
import { parseCatchment, reachableLines, catchmentFile, distance } from '../src/bakedWalking.ts';
...
assert.throws(() => parseCatchment({ ...collection(seg(0, 0, x, 0, 0, 1)), budget: 0 }));
assert.throws(() => parseCatchment({ ...collection(seg(0, 0, x, 0, 0, 1)), gap: -1 }));
assert.throws(() => parseCatchment(collection(seg(0, 0, x, 0, -1, 1))));
assert.throws(() => parseCatchment(collection(seg(0, 0, x, 0, 1001, 1002))));
assert.throws(() => parseCatchment(collection([0, 0, x, 0, 0, 1, 0, 2])));
