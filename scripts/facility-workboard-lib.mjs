import {batchRank} from './facility-routing-lib.mjs';
...
export function groupCandidates(metadata,reviewedKeys){
 ...
 for(const [key,rows] of buckets){
  ...
  for(const [addressKey,articles] of sub){
   ...
   const nameOperator=single?operatorHint(names[0]):null,operatorLinks=articles.flatMap(a=>a.operator_link_hints??[]),domainOperators=unique(operatorLinks.map(h=>h.operator));
   const operator=nameOperator??(names.length<=1&&domainOperators.length===1?domainOperators[0]:null),reviewedIds=unique(articles.flatMap(a=>a.reviewed_facility_hint_ids??[]));
...
export function buildWorkboard(index,reviews,overlay,metadata,caseReviews,checkedAt){
 ...
 for(const c of cases.values())for(const i of c.issues)if(i.impact==='resolved'){
  ...
  const r=i.resolution;assert(r&&typeof r.checked_at==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(r.checked_at)&&Number.isFinite(Date.parse(r.checked_at))&&new Date(r.checked_at).toISOString().slice(0,10)===r.checked_at&&r.checked_at<=checkedAt&&r.reason&&r.evidence_ref&&c.evidence_refs?.some(e=>hash(e)===hash(r.evidence_ref)),'Resolved issue needs checked evidence');
 ...
 const batches=new Map();for(const g of candidates){const reuse=g.reviewed_facility_hint_ids.length>0,key=reuse?'reuse:reviewed-current':g.operator_hint?'operator:'+g.operator_hint:'area:'+g.city;if(!batches.has(key))batches.set(key,{batch_id:key,label:reuse?'照合済み施設との再照合':g.operator_hint??g.city,mode:reuse?'existing_evidence_comparison':g.operator_hint?'official_directory_comparison':'local_store_review',group_ids:[],article_keys:[],existing_official_hint_groups:0,high_priority_groups:0});const b=batches.get(key);b.group_ids.push(g.group_id);b.article_keys.push(...g.article_keys);if(g.official_link_hint_ids.length||g.operator_link_hints.length)b.existing_official_hint_groups++;if(g.priority==='P1')b.high_priority_groups++;}
 const batchRows=[...batches.values()].map(b=>({...b,candidate_groups:b.group_ids.length,article_count:b.article_keys.length})).sort((a,b)=>batchRank(a)-batchRank(b)||b.existing_official_hint_groups-a.existing_official_hint_groups||b.high_priority_groups-a.high_priority_groups||b.article_count-a.article_count||a.batch_id.localeCompare(b.batch_id));
