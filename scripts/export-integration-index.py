...
tables['facilities.csv'] = [dict(id=f['id'], name=f['properties']['name'], city=f['properties']['city'], category=f['properties']['category'],
    ...
    classification_review_status=f['properties'].get('classification_review', {}).get('status', ''),
    classification_review_note=f['properties'].get('classification_review', {}).get('note', ''),
    classification_review_url=f['properties'].get('classification_review', {}).get('evidence_url', ''),
...
manifest = dict(created_at=datetime.date.today().isoformat(), rows={name:len(rows) for name,rows in tables.items()},
