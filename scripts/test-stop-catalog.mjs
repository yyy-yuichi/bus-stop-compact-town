...
import { nationalCatalog, nationalCatchmentId, municipalCatalog, nearbyNationalStops, pilotCatalog } from '../src/stopCatalog.ts';
...
const citySource = read('review-stops.geojson');
const cityBefore = JSON.stringify(citySource);
...
for (const [i, stop] of cityStops.entries()) {
  ...
  assert.deepEqual(stop.geometry, citySource.features[i].geometry);
  assert.deepEqual(stop.properties.routes, citySource.features[i].properties.route_ids.map(id => routes.find(r => r.id === id).name));
  assert.equal(stop.properties.source_kind, 'municipal');
...
const chosen = catalog.features[0];
const municipalOrigin = { ...cityStops[0], geometry: chosen.geometry };
const samePosition = { ...chosen, id: 'mlit-p11-22-35:999999' };
const farAway = { ...chosen, id: 'mlit-p11-22-35:999998', geometry: { type: 'Point', coordinates: [130, 33] } };
assert.equal(nearbyNationalStops(municipalOrigin, [chosen, samePosition, farAway, municipalOrigin], {}).length, 1, 'Offer unique nearby calculation origins, with no distant or city origin');
