...
test('adopted changes preserve the preceding overlay and bind public evidence without requiring private page receipts',()=>{
 ...
 const before={...overlay,updates:overlay.updates.slice(0,26),additions:overlay.additions.slice(0,172)},after={...overlay,updates:overlay.updates.slice(0,30),additions:overlay.additions.slice(0,172)};
 ...
 assert.equal(current.filter(f=>f.properties.freshness_review?.status==='closed').length,9);assert.equal(current.filter(f=>f.properties.duplicate_of).length,1);
});
