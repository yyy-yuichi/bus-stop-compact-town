...
岩国市GTFSについて、当初のREADMEに「65件は親停留所」と記した説明を訂正した。原本を再読込すると800件すべてがlocation_type=0、parent_stationは全件空欄。採用済み735件との差65件は由宇地区の改正未照合による保留である。公開の光市計算・時刻・地図表示への影響はない。原本の種別と採用／保留IDを検査スクリプトで確認した。訂正版ソースZIPと本報告の訂正版を正式保存し、初回のソースZIPより優先する。初回保存物は削除せず履歴として残す。
