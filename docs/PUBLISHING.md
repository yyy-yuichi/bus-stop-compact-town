...
通常マップは国土数値情報4,418件、徒歩圏試作はOSM7地点、買い物先は22件（商業施設19件、スーパー1件、ドラッグストア2件）です。比較マップにはOSM原本1,085地点と市データ907件も収録しています。リポジトリの公開範囲を変更する必要はありません。
...
地図画像・青いバス停・3種類の22施設、名前検索、一覧選択、種類の表示切替、出典説明ページ、データリンクを確認します。最初はindex.htmlとGeoJSONを長期キャッシュせず、データ更新時に古い版が残らないようにしてください。
