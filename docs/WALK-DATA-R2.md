...
2026-09-12の統合後も、通常マップは国の原資料4,418レコードと元の共有IDを維持する。徒歩圏計算用の3,946地点は `public/data/baked-bus-stops.geojson` に収録し、`scripts/build-bus-stops.py` / `scripts/bake-walking.py` はこちらを使う。`bus_stop.geojson` は従来のOSM原本・7地点の試作・比較画面用として保持する。新しい徒歩圏の表示は `src/BakedWalkingPanel.tsx`、形式の読み取りは `src/bakedWalking.ts`。詳しくは [統合記録](isochrone-merge-20260912.md) を参照。
...
本人限定Sitesでは `VITE_WALK_DATA_URL=/data/walk` でビルドし、`preview-worker.js` が公開R2の徒歩圏JSONを同一サイト経由で返す。R2のCORSへ本人限定Sitesのオリジンを追加せず、全徒歩圏をSitesへ同梱もしない。公開GitHub Pagesは従来どおりR2を直接取得する。
